...
//! icon]); min height 184; above `min(756, view.max_height)` the icon/body row scrolls
...
use self::tokens::{theme_fonts, FluentTokens, BODY_SIZE, LINE_RATIO, TITLE_SIZE};
...
impl FluentTheme {
    pub(crate) fn new() -> Self {
        FluentTheme { tokens: FluentTokens::new(&Appearance::default()) }
    }
}
...
impl Theme for FluentTheme {
    ...
    fn configure_style(&self, style: &mut egui::Style) {
        ...
        for w in [&mut style.visuals.widgets.inactive, &mut style.visuals.widgets.hovered, &mut style.visuals.widgets.active] {
            // The thumb colour (`foreground_color = false`: egui paints the handle with `bg_fill`).
            w.bg_fill = tk.scroll_thumb;
            w.corner_radius = egui::CornerRadius::same(3);
        }
        // No scroll bar track (egui paints it in `extreme_bg_color`).
    ...
    fn ui(&self, view: &DialogView<'_>, ui: &mut Ui) -> DialogUiOutput {
        ...
        let labels: Vec<TextBlock> = view.buttons.iter().map(|b| text::layout(&ctx, b, &body_style, f32::INFINITY, Some(1))).collect();
        ...
        let labels: Vec<TextBlock> = labels.into_iter()
                                           .zip(view.buttons)
                                           .map(|(l, b)| if l.size.x > label_max { text::layout(&ctx, b, &body_style, label_max, Some(1)) } else { l })
                                           .collect();
        ...
        // ---- button bar ----
        // The default button: the last API index (core focuses it on open).
        ...
        if n > 0 {
            ...
            let accent = |i: usize| {
                view.progress.is_none() && Some(i) == default_button && (focused_button.is_none() || focused_button == default_button)
            };
...
mod tests {
    ...
    fn buttons_are_reversed_with_the_default_first() {
        ...
        let (out, ()) = run_ui(&ctx, &theme, &view, raw, |_| ());
