...
use crate::backends::egui_core::render::raster::{BufferMutRef, color::SelectedImpl, egui_texture::EguiTexture, render::DrawInfo};

// Upstream generated this dispatcher with the `constify` proc-macro; it is written out by
// hand here (same 8-way const-generic dispatch) so xdialog needs no proc-macro dependency.
pub fn draw_rect(
    ...
    macro_rules! go {
        ($c:literal, $u:literal, $a:literal) => {
            simd_impl.dispatch(|simd_impl| {
                draw_rect_impl::<$c, $u, $a>(simd_impl, buffer, texture, draw)
            })
        };
    }
