    ...
    macro_rules! go {
        ($c:literal, $u:literal, $a:literal) => {
            simd_impl.dispatch(|simd_impl| {
                draw_tri_impl::<SUBPIX_BITS, $c, $u, $a>(simd_impl, buffer, texture, draw)
            })
        };
    }
            ...
            if alpha_blend && !vert_uvs_vary {
                ...
                for ss_x in ss_start..ss_end {
                    let src = if vert_uvs_vary || vert_col_vary {
                        ...
                        crate::backends::egui_core::render::raster::color::GenericImpl.unorm_mult4x4(vert_color, tex_color)
