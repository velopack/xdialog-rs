...
impl<'a, T> WebView<'a, T> {
    ...
    pub fn handle(&self) -> Handle<T> {
        Handle { inner: self.inner.unwrap(), live: Arc::downgrade(&self.user_data_wrapper().live), _phantom: PhantomData }
    }
    ...
    unsafe fn _into_inner(&mut self) -> T {
        let lock = self.user_data_wrapper().live.write().expect("A dispatch channel thread panicked while holding mutex to WebView.");
...
impl<T> Handle<T> {
        ...
        F: FnOnce(&mut WebView<T>) -> WVResult + Send + 'static,
    {
        ...
        let closure: Box<dyn FnOnce(&mut WebView<T>) -> WVResult + Send + 'static> = Box::new(f);
        let _lock = mutex.read().map_err(|_| Error::Dispatch)?;
        let wrapper = Box::new(ClosureWrapper { closure });

        // Send closure to webview.
        unsafe { webview_dispatch(self.inner, Some(ffi_dispatch_handler::<T> as _), Box::into_raw(wrapper) as _) }
        Ok(())
    }
}
...
struct ClosureWrapper<T> {
    closure: Box<dyn FnOnce(&mut WebView<T>) -> Result<(), Error> + Send>,
}
...
extern "C" fn ffi_dispatch_handler<T>(webview: *mut CWebView, arg: *mut c_void) {
    unsafe {
        ...
        let wrapper = Box::from_raw(arg as *mut ClosureWrapper<T>);
        let result = (wrapper.closure)(&mut handle);
