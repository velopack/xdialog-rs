...
//!   SHA-256 equals `tests/visual_references/egui/fluent/FONT_SHA256` (seeding writes that file).
...
fn centre(d: &OffscreenDialog, i: usize) -> (f64, f64) {
    let r = d.button_rects()[i];
    let p = d.ppp() as f64;
    ((r[0] + r[2] / 2.0) as f64 * p, (r[1] + r[3] / 2.0) as f64 * p)
}
...
#[test]
fn fluent_goldens() {
    let font = Path::new(r"C:\Windows\Fonts\SegUIVar.ttf");
    let Ok(bytes) = std::fs::read(font) else {
        println!("skipped: Segoe UI Variable missing or a different version");
        return;
    };
    let hash = sha256_hex(&bytes);
    let record = golden_dir("fluent").join("FONT_SHA256");
    if seeding() {
        std::fs::create_dir_all(golden_dir("fluent")).unwrap();
        std::fs::write(&record, format!("{hash}\n")).unwrap();
    } else if std::fs::read_to_string(&record).map(|s| s.trim().to_owned()).ok().as_deref() != Some(hash.as_str()) {
        println!("skipped: Segoe UI Variable missing or a different version");
        return;
    }
